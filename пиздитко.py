...
<body>
    ...
    <script>
        ...
        async function lookupIP(ip) {
            try {
                const response = await fetch(`https://ipapi.co/${ip}/json/`);  // Замени `ipapi.co` на `ip-api.com`, если `ipapi.co` не работает
                const data = await response.json();

                if (data.error) {
                    document.getElementById('result').textContent = `Ошибка: ${data.reason}`;
                } else {
                    let mapURL = `https://www.google.com/maps/search/?api=1&query=${data.latitude},${data.longitude}`;
                    const resultHTML = `
                        Ip        : ${data.ip}<br>
                        Status    : Valid<br>
                        ...
                        Org       : ${data.asn}<br>
                        As        : ${data.asn}<br>
                        Открыть карту: <a href="${mapURL}" target="_blank">Открыть карту</a>
                    `;
